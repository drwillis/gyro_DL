...
def get_dataset(seed=0, trials=50, test_split=0.5, save_dir=None, actions=np.array([[0, 0]], dtype=float), dt=0.05,
    ...
    try:
        ...
        for actionIdx in range(actions.shape[0]):
            ...
            trajs, tspan, _ = sample_gym(seed=seed, trials=trials, actions=np.array([action]), ori_rep = ori_rep, friction = friction, dt=dt, **kwargs)
...
if __name__ == "__main__":
    ...
    data = get_dataset(seed=0, timesteps=timesteps, save_dir='data', actions=actions, ori_rep='rotmat', trials=50, dt=dt)
