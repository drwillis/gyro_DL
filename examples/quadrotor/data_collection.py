...
parser = argparse.ArgumentParser(
    description='Debugging script for PyBullet applyExternalForce() and applyExternalTorque() PyBullet')
parser.add_argument('--duration_sec', default=2.5, type=float,
                    help='Duration of the simulation in seconds (default: 30)', metavar='')
parser.add_argument('--num_resets', default=1, type=int,
                    help='Number of times the simulation is reset to its initial conditions (default: 2)', metavar='')
        ...
        for i in range(int(ARGS.duration_sec * ARGS.control_freq_hz)):
            ...
            for s in range(AGGR_PHY_STEPS):
                ...
                collected_state = np.concatenate(
                    (state[0:3], rotmat.flatten(), v_bodyframe, w_bodyframe, thrust_torques))
