...
class GyroEnvV1(gym.Env):
    ...
    def __init__(self, g=10.0, ori_rep='angle', friction=False):
        ...
        self.Jzz = 0.1  # moment of inertia of vibrating mass about rotational axis
        self.ky = 10.0  # spring constant in vibrating direction
        self.by = 0.1  # viscous friction coefficient in vibrating direction
        self.kx = 10  # spring constant in sensing direction
        self.bx = 1.0  # viscous friction coefficient in sensing direction
        self.y0 = 0.1  # amplitude of excitation signal
        ...
        self.params = np.array([self.m, self.Jzz, self.ky, self.ky, self.kx, self.bx, self.y0, self.wy, self.cy,
                                self.cx])  # Group constants into a vector to send to ODE function
        ...
        self.action_space = spaces.Box(low=np.float32([-self.max_torque, -self.max_amplitude]),
                                       high=np.float32([self.max_torque, self.max_amplitude]), shape=(2,),
                                       dtype=np.float32)
    ...
    def dynamics(self, t, b, action):
        ...
        input_torque = action[0]
        input_force_y = action[1]
        F = np.array([0,  # no external force applied in sensing direction (x)
                      # self.y0 * np.cos(self.wy * t),  # excitation signal applied in vibrating direction (y)
                      input_force_y,  # excitation signal applied in vibrating direction (y)
                      input_torque]).T  # torque applied to the gyroscope (so that it rotates)
